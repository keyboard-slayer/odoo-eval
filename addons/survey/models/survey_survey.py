...
class Survey(models.Model):
    ...
    _access_token_unique = models.Constraint(
        'unique(access_token)',
        lambda env: env._('Access token should be unique'),
    )
    _session_code_unique = models.Constraint(
        'unique(session_code)',
        lambda env: env._('Session code should be unique'),
    )
    _certification_check = models.Constraint(
        "CHECK( scoring_type!='no_scoring' OR certification=False )",
        lambda env: env._('You can only create certifications for surveys that have a scoring mechanism.'),
    )
    _scoring_success_min_check = models.Constraint(
        ...
        lambda env: env._('The percentage of success has to be defined between 0 and 100.'),
    )
    _time_limit_check = models.Constraint(
        'CHECK( (is_time_limited=False) OR (time_limit is not null AND time_limit > 0) )',
        lambda env: env._('The time limit needs to be a positive number if the survey is time limited.'),
    )
    _attempts_limit_check = models.Constraint(
        ...
        lambda env: env._('The attempts limit needs to be a positive number if the survey has a limited number of attempts.'),
    )
    _badge_uniq = models.Constraint(
        'unique (certification_badge_id)',
        lambda env: env._('The badge for each survey should be unique!'),
    )
    _session_speed_rating_has_time_limit = models.Constraint(
        ...
        lambda env: env._('A positive default time limit is required when the session rewards quick answers.'),
    )
