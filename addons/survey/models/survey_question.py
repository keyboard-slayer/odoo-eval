...
class SurveyQuestion(models.Model):
    ...
    _positive_len_min = models.Constraint(
        ...
        lambda env: env._('A length must be positive!'),
    )
    ...
    _validation_length = models.Constraint(
        ...
        lambda env: env._('Max length cannot be smaller than min length!'),
    )
    _validation_float = models.Constraint(
        'CHECK (validation_min_float_value <= validation_max_float_value)',
        lambda env: env._('Max value cannot be smaller than min value!'),
    )
    _validation_date = models.Constraint(
        'CHECK (validation_min_date <= validation_max_date)',
        lambda env: env._('Max date cannot be smaller than min date!'),
    )
    _validation_datetime = models.Constraint(
        'CHECK (validation_min_datetime <= validation_max_datetime)',
        lambda env: env._('Max datetime cannot be smaller than min datetime!'),
    )
    _positive_answer_score = models.Constraint(
        'CHECK (answer_score >= 0)',
        lambda env: env._('An answer score for a non-multiple choice question cannot be negative!'),
    )
    _scored_datetime_have_answers = models.Constraint(
        ...
        lambda env: env._('All "Is a scored question = True" and "Question Type: Datetime" questions need an answer'),
    )
    _scored_date_have_answers = models.Constraint(
        ...
        lambda env: env._('All "Is a scored question = True" and "Question Type: Date" questions need an answer'),
    )
    _scale = models.Constraint(
        "CHECK (question_type != 'scale' OR (scale_min >= 0 AND scale_max <= 10 AND scale_min < scale_max))",
        lambda env: env._('The scale must be a growing non-empty range between 0 and 10 (inclusive)'),
    )
    _is_time_limited_have_time_limit = models.Constraint(
        'CHECK (is_time_limited != TRUE OR time_limit IS NOT NULL AND time_limit > 0)',
        lambda env: env._('All time-limited questions need a positive time limit'),
    )
...
class SurveyQuestionAnswer(models.Model):
    ...
    _value_not_empty = models.Constraint(
        ...
        lambda env: env._('Suggested answer value must not be empty (a text and/or an image must be provided).'),
    )
