...
class Survey(http.Controller):
    ...
    def _fetch_from_access_token(self, survey_token, answer_token, survey_id=False):
        ...
        if survey_id:
            survey_sudo = request.env['survey.survey'].with_context(active_test=False).sudo().browse(survey_id).exists()
        else:
    ...
    def _check_validity(self, survey_token, answer_token, ensure_token=True, check_partner=True, survey_id=False):
        ...
        """
        survey_sudo, answer_sudo = self._fetch_from_access_token(survey_token, answer_token, survey_id=survey_id)
    ...
    def _get_access_data(self, survey_token, answer_token, ensure_token=True, check_partner=True, survey_id=False):
        ...
        validity_code = self._check_validity(survey_token, answer_token, ensure_token=ensure_token, check_partner=check_partner, survey_id=survey_id)
    ...
    def _redirect_with_error(self, access_data, error_key):
        ...
        if error_key == 'survey_void' and access_data['can_answer']:
            ...
            if not answer_sudo:  # survey is not even started
                redirect_url = f'/web/login?redirect=/survey/start/{survey_sudo.id}/{survey_sudo.access_token}'
                ...
                if answer_sudo.partner_id and (answer_sudo.partner_id.user_ids or survey_sudo.users_can_signup):
                    ...
                    redirect_url = answer_sudo.partner_id._get_signup_url_for_action(url=f'/survey/start/{survey_sudo.id}/{survey_sudo.access_token}?answer_token={answer_sudo.access_token}')[answer_sudo.partner_id.id]
                else:
                    redirect_url = '/web/login?redirect=%s' % (f'/survey/start/{survey_sudo.id}/{survey_sudo.access_token}?answer_token={answer_sudo.access_token}')
    ...
    # ------------------------------------------------------------
    @http.route(['/survey/test/<int:survey_id>/<string:survey_token>',
                '/survey/test/<string:survey_token>'
                ], type='http', auth='user', website=True)
    def survey_test(self, survey_token, survey_id=False, **kwargs):
        """ Test mode for surveys: create a test answer, only for managers or officers
        testing their surveys """
        survey_sudo, __ = self._fetch_from_access_token(survey_token, False, survey_id)
        try:
            answer_sudo = survey_sudo._create_answer(user=request.env.user, test_entry=True)
        except:
            return request.redirect('/')
        return request.redirect('/survey/start/%s/%s?%s' % (survey_sudo.id, survey_sudo.access_token, keep_query('*', answer_token=answer_sudo.access_token)))

    @http.route(['/survey/retry/<int:survey_id>/<string:survey_token>/<string:answer_token>',
                 '/survey/retry/<string:survey_token>/<string:answer_token>'
                ], type='http', auth='public', website=True)
    def survey_retry(self, survey_token, answer_token, survey_id=False, **post):
        """ This route is called whenever the user has attempts left and hits the 'Retry' button
        after failing the survey."""
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=True, survey_id=survey_id)
        ...
        try:
            ...
            return request.redirect("/")
        return request.redirect('/survey/start/%s/%s?%s' % (survey_sudo.id, survey_sudo.access_token, keep_query('*', answer_token=retry_answer_sudo.access_token)))
    ...
    @http.route(['/survey/start/<string:survey_token>',
                 '/survey/start/<int:survey_id>/<string:survey_token>'
                ], type='http', auth='public', website=True)
    def survey_start(self, survey_token, answer_token=None, email=False, survey_id=False, **post):
        ...
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=False, survey_id=survey_id)
        ...
        if answer_from_cookie and access_data['validity_code'] in ('answer_wrong_user', 'token_wrong'):
            ...
            access_data = self._get_access_data(survey_token, None, ensure_token=False, survey_id=survey_id)
        ...
        return request.redirect('/survey/%s/%s/%s' % (survey_sudo.id, survey_sudo.access_token, answer_sudo.access_token))
    ...
    @http.route(['/survey/<int:survey_id>/<string:survey_token>/<string:answer_token>',
                 '/survey/<string:survey_token>/<string:answer_token>'
                ], type='http', auth='public', website=True)
    def survey_display_page(self, survey_token, answer_token, survey_id=False, **post):
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=True, survey_id=survey_id)
        if access_data['validity_code'] is not True:
            return self._redirect_with_error(access_data, access_data['validity_code'])
    ...
    @http.route(['/survey/<int:survey_id>/<string:survey_token>/get_background_image',
                 '/survey/<string:survey_token>/get_background_image'
                ], type='http', auth="public", website=True, sitemap=False)
    def survey_get_background(self, survey_token, survey_id=False):
        survey_sudo, __ = self._fetch_from_access_token(survey_token, False, survey_id)
        return request.env['ir.binary']._get_image_stream_from(
            survey_sudo, 'background_image'
        ).get_response()

    @http.route(['/survey/<int:survey_id>/<string:survey_token>/<int:section_id>/get_background_image',
                 '/survey/<string:survey_token>/<int:section_id>/get_background_image'
                ], type='http', auth="public", website=True, sitemap=False)
    def survey_section_get_background(self, survey_token, section_id, survey_id=False):
        survey_sudo, __ = self._fetch_from_access_token(survey_token, False, survey_id)
    ...
    @http.route(['/survey/get_question_image/<int:survey_id>/<string:survey_token>/<string:answer_token>/<int:question_id>/<int:suggested_answer_id>',
                 '/survey/get_question_image/<string:survey_token>/<string:answer_token>/<int:question_id>/<int:suggested_answer_id>'
                ], type='http', auth="public", website=True, sitemap=False)
    def survey_get_question_image(self, survey_token, answer_token, question_id, suggested_answer_id, survey_id=False):
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=True, survey_id=survey_id)
        if access_data['validity_code'] is not True:
    ...
    @http.route(['/survey/begin/<int:survey_id>/<string:survey_token>/<string:answer_token>',
                 '/survey/begin/<string:survey_token>/<string:answer_token>'
                ], type='json', auth='public', website=True)
    def survey_begin(self, survey_token, answer_token, survey_id=False, **post):
    ...
    @http.route(['/survey/next_question/<int:survey_id>/<string:survey_token>/<string:answer_token>',
                 '/survey/next_question/<string:survey_token>/<string:answer_token>'
                ], type='json', auth='public', website=True)
    def survey_next_question(self, survey_token, answer_token, survey_id=False, **post):
    ...
    @http.route(['/survey/submit/<int:survey_id>/<string:survey_token>/<string:answer_token>',
                 '/survey/submit/<string:survey_token>/<string:answer_token>'
                ], type='json', auth='public', website=True)
    def survey_submit(self, survey_token, answer_token, survey_id=False, **post):
    ...
    @http.route(['/survey/print/<int:survey_id>/<string:survey_token>',
                 '/survey/print/<string:survey_token>'
                ], type='http', auth='public', website=True, sitemap=False)
    def survey_print(self, survey_token, review=False, answer_token=None, survey_id=False, **post):
        '''Display an survey in printable view; if <answer_token> is set, it will
        grab the answers of the user_input_id that has <answer_token>.'''
        access_data = self._get_access_data(survey_token, answer_token, ensure_token=False, check_partner=False, survey_id=survey_id)
    ...
    @http.route(['/survey/<int:survey_id>/get_certification',
                 '/survey/get_certification'
                ], type='http', auth='user', methods=['GET'], website=True)
    def survey_get_certification(self, survey_id=False, **kwargs):
