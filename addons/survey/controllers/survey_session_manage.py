...
class UserInputSession(http.Controller):
    ...
    # DANE: searching on survey_token is depreciated and it will be removed in v19/20.
    # Searching only have to be done on survey_id.
    ...
    # DANE: remove session_code in v19/20
    def _fetch_from_session_code(self, session_code=False, survey_id=False):
        ...
        if survey_id:
            ...
            survey = request.env['survey.survey'].sudo().search([('session_code', '=', session_code)])
    ...
    @http.route(['/survey/session/manage/<int:survey_id>/<string:survey_token>',
    ...
    @http.route(['/survey/session/next_question/<int:survey_id>/<string:survey_token>',
    ...
    @http.route(['/survey/session/results/<int:survey_id>/<string:survey_token>',
    ...
    @http.route(['/s/<string:session_code>', '/s/<int:survey_id>/<string:session_code>'], type='http', auth='public', website=True)
    ...
    @http.route(['/survey/check_session_code/<string:session_code>',
                 '/survey/check_session_code/<int:survey_id>/<string:session_code>'
