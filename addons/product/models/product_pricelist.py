...
class Pricelist(models.Model):
    ...
    def _get_partner_pricelist_multi(self, partner_ids):
        ...
        for pid in partner_ids:
            rid = pid or pid.origin
            if (p := specific_properties.get(rid)) and p._get_partner_pricelist_multi_filter_hook():
                result[pid] = specific_properties[rid]
            else:
                remaining_partner_ids.append(pid)

        if not remaining_partner_ids:
            return result
        ...
        partners_by_country = self.env['res.partner'].browse(remaining_partner_ids).grouped('country_id')
        for country, partners in partners_by_country.items():
            ...
            result.update(dict.fromkeys(partners._ids, pl or pl_fallback))
