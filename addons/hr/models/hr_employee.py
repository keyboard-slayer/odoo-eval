...
class HrEmployeePrivate(models.Model):
    ...
    _barcode_uniq = models.Constraint(
        'unique (barcode)',
        lambda env: env._('The Badge ID must be unique, this one is already assigned to another employee.'),
    )
    _user_uniq = models.Constraint(
        'unique (user_id, company_id)',
        lambda env: env._('A user cannot be linked to multiple employees in the same company.'),
    )
