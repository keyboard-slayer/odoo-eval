...
class Job(models.Model):
    ...
    _name_company_uniq = models.Constraint(
        ...
        lambda env: env._('The name of the job position must be unique per department in company!'),
    )
    _no_of_recruitment_positive = models.Constraint(
        'CHECK(no_of_recruitment >= 0)',
        lambda env: env._('The expected number of new employees must be positive.'),
    )
