...
export class LivechatCoreWeb {
    ...
    constructor(env, services) {
        ...
        this.busService.subscribe("im_livechat.channel/leave", async (payload) => {
            const { channel, removed_by_user_id: removedByUserId } = payload;
            const thread = this.store.Thread.insert(channel);
            await thread.fetchLivechatChannelInfo();
            if (removedByUserId && removedByUserId !== this.store.self.userId) {
                this.notificationService.add(
                    _t("You have been removed from #%s", thread.displayName),
                    { type: "info" }
                );
            }
        });
        this.busService.subscribe("im_livechat.channel/delete", (payload, metadata) => {
     ...
     */
    _handleNotificationChannelDelete(thread) {
