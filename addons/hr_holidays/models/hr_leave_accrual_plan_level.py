...
class AccrualPlanLevel(models.Model):
    ...
    _check_dates = models.Constraint(
        ...
        lambda env: env._("The dates you've set up aren't correct. Please check them."),
    )
    _start_count_check = models.Constraint(
        'CHECK( start_count >= 0 )',
        lambda env: env._('You can not start an accrual in the past.'),
    )
    _added_value_greater_than_zero = models.Constraint(
        'CHECK(added_value > 0)',
        lambda env: env._('You must give a rate greater than 0 in accrual plan levels.'),
    )
    _valid_yearly_cap_value = models.Constraint(
        'CHECK(cap_accrued_time_yearly IS NOT TRUE OR COALESCE(maximum_leave_yearly, 0) > 0)',
        lambda env: env._('You cannot have a cap on yearly accrued time without setting a maximum amount.'),
    )
