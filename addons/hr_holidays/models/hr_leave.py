...
class HolidaysRequest(models.Model):
    ...
    _date_check2 = models.Constraint(
        ...
        lambda env: env._('The start date must be before or equal to the end date.'),
    )
    _date_check3 = models.Constraint(
        'CHECK ((request_date_from <= request_date_to))',
        lambda env: env._('The request start date must be before or equal to the request end date.'),
    )
    _duration_check = models.Constraint(
        'CHECK ( number_of_days >= 0 )',
        lambda env: env._("If you want to change the number of days you should use the 'period' mode"),
    )
