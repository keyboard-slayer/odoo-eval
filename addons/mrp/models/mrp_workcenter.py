...
class WorkcenterTag(models.Model):
    ...
    _tag_name_unique = models.Constraint(
        'unique(name)',
        lambda env: env._('The tag name must be unique.'),
    )
...
class MrpWorkCenterCapacity(models.Model):
    ...
    _positive_capacity = models.Constraint(
        'CHECK(capacity > 0)',
        lambda env: env._('Capacity should be a positive number.'),
    )
    _unique_product = models.Constraint(
        'UNIQUE(workcenter_id, product_id)',
        lambda env: env._('Product capacity should be unique for each workcenter.'),
    )
