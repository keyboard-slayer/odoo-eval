...
class MrpWorkorder(models.Model):
    ...
    def _compute_state(self):
        ...
        for workorder in self:
            if workorder.state in ('done', 'cancel'):
                continue
            blocked = workorder._are_previous_workorder_blocking()
            if workorder.state == 'progress' and not blocked:
                continue
            has_qty_ready = workorder._has_qty_ready()
    ...
    def _compute_qty_ready(self):
        for workorder in self:
            ...
            if not workorder.blocked_by_workorder_ids:
                ...
                continue
            workorder.qty_ready = min((wo.qty_produced + wo.qty_reported_from_previous_wo if wo.state != 'cancel' else workorder.qty_remaining + workorder.qty_produced for wo in workorder.blocked_by_workorder_ids), default=workorder.qty_remaining + workorder.qty_produced) - workorder.qty_produced
    ...
    def _has_qty_ready(self):
        return float_compare(self.qty_ready, 0, precision_rounding=self.product_uom_id.rounding) > 0
