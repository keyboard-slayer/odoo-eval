...
class TestPortalMessageUpdateController(TestMessageUpdateControllerCommon):
    def _execute_test_message_update_for_record(self, record):
    ...
    def test_message_update_portal_no_partner(self):
        """Test only admin and portal user can update a portal user message on a record without assigned partner."""
        record = self.env["mail.test.portal.no.partner"].create({"name": "Test"})
        self._execute_test_message_update_for_record(record)

    def test_message_message_update_portal_assigned_partner(self):
        """Test only admin and portal user can update a portal user message on a record with assigned partner."""
