...
class PackageType(models.Model):
    ...
    _barcode_uniq = models.Constraint(
        'unique(barcode)',
        lambda env: env._('A barcode can only be assigned to one package type!'),
    )
    _positive_height = models.Constraint(
        'CHECK(height>=0.0)',
        lambda env: env._('Height must be positive'),
    )
    _positive_width = models.Constraint(
        'CHECK(width>=0.0)',
        lambda env: env._('Width must be positive'),
    )
    _positive_length = models.Constraint(
        'CHECK(packaging_length>=0.0)',
        lambda env: env._('Length must be positive'),
    )
    _positive_max_weight = models.Constraint(
        'CHECK(max_weight>=0.0)',
        lambda env: env._('Max Weight must be positive'),
    )
