...
class Location(models.Model):
    ...
    _barcode_company_uniq = models.Constraint(
        ...
        lambda env: env._('The barcode for a location must be unique per company!'),
    )
    _inventory_freq_nonneg = models.Constraint(
        'check(cyclic_inventory_frequency >= 0)',
        lambda env: env._('The inventory frequency (days) for a location must be non-negative'),
    )
