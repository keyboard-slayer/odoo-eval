...
class SlidePartnerRelation(models.Model):
    ...
    _slide_partner_uniq = models.Constraint(
        ...
        lambda env: env._('A partner membership to a slide must be unique!'),
    )
    _check_vote = models.Constraint(
        'CHECK(vote IN (-1, 0, 1))',
        lambda env: env._('The vote must be 1, 0 or -1.'),
    )
...
class SlideTag(models.Model):
    ...
    _slide_tag_unique = models.Constraint(
        'UNIQUE(name)',
        lambda env: env._('A tag must be unique!'),
    )
...
class Slide(models.Model):
    ...
    _exclusion_html_content_and_url = models.Constraint(
        ...
        lambda env: env._('A slide is either filled with a url or HTML content. Not both.'),
    )
