...
class ChannelUsersRelation(models.Model):
    ...
    _channel_partner_uniq = models.Constraint(
        ...
        lambda env: env._('A partner membership to a channel must be unique!'),
    )
    _check_completion = models.Constraint(
        'check(completion >= 0 and completion <= 100)',
        lambda env: env._('The completion of a channel is a percentage and should be between 0% and 100.'),
    )
...
class Channel(models.Model):
    ...
    _check_enroll = models.Constraint(
        ...
        lambda env: env._("The Enroll Policy should be set to 'On Invitation' when visibility is set to 'Course Attendees'"),
    )
