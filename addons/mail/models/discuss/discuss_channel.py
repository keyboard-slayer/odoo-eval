...
class Channel(models.Model):
    ...
    _channel_type_not_null = models.Constraint(
        ...
        lambda env: env._('The channel type cannot be empty'),
    )
    _from_message_id_unique = models.Constraint(
        'UNIQUE(from_message_id)',
        lambda env: env._('Messages can only be linked to one sub-channel'),
    )
    _sub_channel_no_group_public_id = models.Constraint(
        'CHECK(parent_channel_id IS NULL OR group_public_id IS NULL)',
        lambda env: env._('Group public id should not be set on sub-channels as access is based on parent channel'),
    )
    _uuid_unique = models.Constraint(
        'UNIQUE(uuid)',
        lambda env: env._('The channel UUID must be unique'),
    )
    _group_public_id_check = models.Constraint(
        "CHECK (channel_type = 'channel' OR group_public_id IS NULL)",
        lambda env: env._('Group authorization and group auto-subscription are only supported on channels.'),
    )
