...
class ScheduledMessage(models.Model):
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not (model := vals.get('model') or self.env.context.get('default_model')):
                raise ValueError(_('"model" is required on "mail.scheduled.message"'))
            if not (res_id := vals.get('res_id') or self.env.get('default_res_id')):
                raise ValueError(_('"res_id" is required on "mail.scheduled.message"'))
            self._check({
                'model': model,
                'res_id': res_id,
            })

        scheduled_messages = super().create(vals_list)
