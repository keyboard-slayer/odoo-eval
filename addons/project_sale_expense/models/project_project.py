...
class Project(models.Model):
    ...
    def _get_expenses_profitability_items(self, with_action=True):
        ...
        for expense in expenses:
            ...
            if can_see_expense:
                expense_ids.extend(expense.ids)
            dict_amount_per_currency[expense.currency_id] += expense.untaxed_amount_currency * analytic_contribution

        amount_billed = 0.0
        for currency, untaxed_amount_currency in dict_amount_per_currency.items():
            amount_billed += currency._convert(untaxed_amount_currency, self.currency_id, self.company_id, round=False)
