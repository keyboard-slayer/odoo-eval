...
from odoo import SUPERUSER_ID, _, _lt, api, fields, models
...
# TODO VFE rename website_sale_current_pl into another clearer key, like pricelist_id
...
class Website(models.Model):
    ...
    def _create_cart(self):
        ...
        # Reset the environment to the right user
    ...
    def _get_and_cache_current_order(self):
        ...
        if CART_SESSION_CACHE_KEY in request.session:
            ...
            if sale_order_sudo and not self.env.user._is_public():
                if sale_order_sudo._is_anonymous_cart():
                    # If customer logged in, their current cart has to be re-assigned to their partner.
                    sale_order_sudo._update_address(self.env.user.partner_id.id, ['partner_id'])
                elif sale_order_sudo.partner_id != self.env.user.partner_id:
                    # FTR, this cannot happen in real life flows, but is a bad behavior in tests
                    # that is better caught here to ensure tests are following correct use-cases
                    self.sale_reset()
                    sale_order_sudo = SaleOrderSudo
            ...
            if abandonned_cart_sudo:
                if not request.env.cr.readonly:
                    # Force the recomputation of the pricelist and fiscal position when resurrecting an
                    # abandonned cart
        ...
        if sale_order_sudo or not self.env.user._is_public():
            # Do not update the session if there is nothing to gain from it
            request.session['sale_order_id'] = sale_order_sudo.id
    ...
    def sale_reset(self):
        ...
        request.session['website_sale_cart_quantity'] = False
        request.session.pop(PRICELIST_SESSION_CACHE_KEY, None)
