...
class AccountTax(models.Model):
    ...
    def _prepare_base_line_for_taxes_computation(self, record, **kwargs):
        ...
        return {
            **kwargs,
            ...
            # Accounting stuff for tax lines.
    ...
    @api.model
    def _add_base_line_tax_details(self, base_line, company, rounding_method=None):
    ...
    @api.model
    def _add_base_lines_tax_details(self, base_lines, company):
    ...
    @api.model
    def _add_base_line_accounting_tax_details(self, base_line, company, include_caba_tags=False):
        ...
        for tax_data in taxes_data:
            ...
            for tax_rep in tax_reps:
                tax_rep_data = {
                    'tax_rep': tax_rep,
                    'tax_amount_currency': tax_data['tax_amount_currency'] * tax_rep.factor * tax_rep_sign,
                    'tax_amount': tax_data['tax_amount'] * tax_rep.factor * tax_rep_sign,
                    'account': tax_rep._get_aml_target_tax_account(force_caba_exigibility=include_caba_tags) or base_line['account_id'],
                }
                total_tax_amount_currency += tax_rep_data['tax_amount_currency']
                total_tax_amount += tax_rep_data['tax_amount']
                total_factor += tax_rep.factor
                if rounding_method == 'round_per_line':
                    tax_rep_data['tax_amount_currency'] = currency.round(tax_rep_data['tax_amount_currency'])
                    tax_rep_data['tax_amount'] = company.currency_id.round(tax_rep_data['tax_amount'])
                tax_reps_data.append(tax_rep_data)

            # Round when 100%.
            if total_factor == 1.0:
                biggest_tax_rep = min(tax_reps_data, key=lambda tax_rep_values: sign * tax_rep_values['tax_amount_currency'])
                biggest_tax_rep['tax_amount_currency'] += tax_data['tax_amount_currency'] - total_tax_amount_currency
                biggest_tax_rep['tax_amount'] += tax_data['tax_amount'] - total_tax_amount
    ...
    def _add_base_lines_accounting_tax_details(self, base_lines, company, include_caba_tags=False):
        for base_line in base_lines:
            self._add_base_line_accounting_tax_details(base_line, company, include_caba_tags=include_caba_tags)
    ...
    def compute_all(self, price_unit, currency=None, quantity=1.0, product=None, partner=None, is_refund=False, handle_price_include=True, include_caba_tags=False, rounding_method=None):
        ...
        self._add_base_line_tax_details(base_line, company, rounding_method=rounding_method)
        self._add_base_line_accounting_tax_details(base_line, company, include_caba_tags=include_caba_tags)
