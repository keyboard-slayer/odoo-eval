...
class SaleStockReplenishmentWarehouseSelect(models.TransientModel):
    ...
    wh_replenishment_option_ids = fields.One2many('stock.replenishment.option', compute='_compute_wh_replenishment_options')
    ...
    def order_avbl(self, route_id, free_qty):
        ...
        so_line.route_id = route_id
        so_line.product_uom_qty = free_qty
        return {'type': 'ir.actions.client', 'tag': 'soft_reload'}

    def order_all(self, route_id):
        self.sale_order_line_id.route_id = route_id
        return {'type': 'ir.actions.client', 'tag': 'soft_reload'}
