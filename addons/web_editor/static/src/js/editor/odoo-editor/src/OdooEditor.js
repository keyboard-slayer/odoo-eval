...
export class OdooEditor extends EventTarget {
    ...
    _setSelectionInTable(ev) {
        ...
        const tds = [...startTable.querySelectorAll('tr')]
                    ...
                    .map(tr => [...tr.querySelectorAll('td')].filter(td => closestElement(td, 'table') === startTable));
        ...
        if (targetTd) {
            ev.preventDefault();
            setSelection(startTd, 0, targetTd, 0);
        } else if (targetNode) {
            ev.preventDefault();
            setSelection(selection.anchorNode, selection.anchorOffset, targetNode, 0);
        }
    }
    _setSingleCellSelection(ev = undefined, sel, td) {
        ...
        if (focusNode.nodeType === Node.TEXT_NODE) {
            const lastTextChild = descendants(td).filter(node => node.nodeType === 3).pop();
            const firstTextChild = descendants(td).filter(node => node.nodeType === 3).shift();
            switch (ev.key) {
                case 'ArrowRight': {
