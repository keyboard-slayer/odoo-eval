...
class AccountEdiXmlUBL21JO(models.AbstractModel):
        ...
        for grouping_key, vals in taxes_vals['tax_details'].items():
            if grouping_key['tax_amount_type'] != 'fixed':
                subtotal = {
                    ...
                    'tax_amount': vals['base_amount'] * vals['tax_category_percent'] / 100,
    ...
    def _get_invoice_line_price_vals(self, line):
        ...
        return {
            **vals,
            'price_amount': round((abs(line.balance) + line._get_discount_amount_jod()) / line.quantity, 10),
    ...
    def _get_invoice_line_tax_totals_vals_list(self, line, taxes_vals):
        ...
        vals = self._get_invoice_tax_totals_vals_list_helper(invoice, taxes_vals)
        vals = [{**val, 'rounding_amount': round(line.price_total / (line.currency_rate or 1), 10)} for val in vals]
        ...
        for grouping_key, tax_details_vals in taxes_vals['tax_details'].items():
            if grouping_key['tax_amount_type'] == 'fixed':
                subtotal = {
                    'currency': JO_CURRENCY,
                    'currency_dp': self._get_currency_decimal_places(),
                    'taxable_amount': tax_details_vals['base_amount'],
                    'tax_amount': tax_details_vals['tax_amount'],
                    'tax_category_vals': tax_details_vals['_tax_category_vals_'],
                }
                vals[0]['tax_subtotal_vals'][0]['taxable_amount'] -= tax_details_vals['tax_amount']
                vals[0]['tax_subtotal_vals'].insert(0, subtotal)
        return vals
    ...
    def _get_invoice_monetary_total_vals(self, invoice, taxes_vals, line_extension_amount, allowance_total_amount, charge_total_amount):
        ...
        return {
            'currency': JO_CURRENCY,
            'currency_dp': self._get_currency_decimal_places(),
            'tax_exclusive_amount': abs(invoice.amount_untaxed_signed) + discount,
            'tax_inclusive_amount': abs(invoice.amount_total_signed),
            'allowance_total_amount': discount,
            'payable_amount': abs(invoice.amount_total_signed),
