...
class ResourceCalendar(models.Model):
    ...
    def plan_days(self, days, day_dt, compute_leaves=False, domain=None):
        ...
        for start, stop, meta in self._iter_intervals(day_dt, direction, compute_leaves, domain):
            ...
            if remaining_days <= 0:
                # Handle fractional interval
                fraction_within_interval = (day_fraction + remaining_days) / day_fraction
                if direction > 0:
                    return revert(start + timedelta(hours=interval_hours * fraction_within_interval))
                else:
                    return revert(stop - timedelta(hours=interval_hours * fraction_within_interval))
        return False
