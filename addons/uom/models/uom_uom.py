...
class UoM(models.Model):
    ...
    _factor_gt_zero = models.Constraint(
        'CHECK (factor!=0)',
        lambda env: env._('The conversion ratio for a unit of measure cannot be 0!'),
    )
    _rounding_gt_zero = models.Constraint(
        'CHECK (rounding>0)',
        lambda env: env._('The rounding precision must be strictly positive.'),
    )
    _factor_reference_is_one = models.Constraint(
        "CHECK((uom_type = 'reference' AND factor = 1.0) OR (uom_type != 'reference'))",
        lambda env: env._('The reference unit must have a conversion factor equal to 1.'),
    )
