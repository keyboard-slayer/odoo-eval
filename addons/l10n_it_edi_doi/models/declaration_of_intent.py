...
class L10nItDeclarationOfIntent(models.Model):
    ...
    _protocol_number_unique = models.Constraint(
        ...
        lambda env: env._('The Protocol Number of a Declaration of Intent must be unique! Please choose another one.'),
    )
    _threshold_positive = models.Constraint(
        'CHECK(threshold > 0)',
        lambda env: env._('The Threshold of a Declaration of Intent must be positive.'),
    )
