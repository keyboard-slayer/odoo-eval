...
class AccountMove(models.Model):
    ...
    #             move.l10n_rs_tax_date_obligations_code = '0'
    ...
    def _compute_show_reset_to_draft_button(self):
        ...
        for move in self:
            move.show_reset_to_draft_button = not move.l10n_rs_edi_state in ('sent', 'to_send') and move.show_reset_to_draft_button
    ...
    def _l10n_rs_edi_send(self, xml):
        # sends the invoice to the eFaktura
        self.ensure_one()
        # attachment_values = self._l10n_rs_edi_get_attachment_values(xml)
        # self.env['ir.attachment'].create(attachment_values)
        # self.invalidate_recordset(fnames=['l10n_rs_edi_attachment_id', 'l10n_rs_edi_attachment_file'])
        # self.with_context(no_new_invoice=True).message_post(attachment_ids=self.l10n_rs_edi_attachment_id.ids)
        params = {'request_id': self.l10n_rs_edi_uuid}
        if not self.company_id.l10n_rs_edi_api_key:
        ...
        try:
            response = requests.post(EFAKTURA_URL, params=params, headers=headers, data=xml)
        except Exception as e:
            self.l10n_rs_edi_state = 'sending_failed'
            self.l10n_rs_edi_error = e
