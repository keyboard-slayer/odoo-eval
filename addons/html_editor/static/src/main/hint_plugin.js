...
import { selectElements, closestElement } from "@html_editor/utils/dom_traversal";
    ...
    updateHints() {
        ...
        if (editableSelection.isCollapsed) {
            for (const hint of this.resources["hints"]) {
                if (hint.selector) {
                    const el = closestElement(editableSelection.anchorNode, hint.selector);
                    if (el && !isProtected(el) && isEmptyBlock(el)) {
                        if (this.hint) {
                            this.removeHint(this.hint);
                        }
