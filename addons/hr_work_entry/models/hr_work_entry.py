...
class HrWorkEntry(models.Model):
    ...
    _work_entry_has_end = models.Constraint(
        ...
        lambda env: env._('Work entry must end. Please define an end date or a duration.'),
    )
    _work_entry_start_before_end = models.Constraint(
        'CHECK (date_stop > date_start)',
        lambda env: env._('Starting time should be before end time.'),
    )
    _work_entries_no_validated_conflict = models.Constraint(
        ...
        """,
        lambda env: env._('Validated work entries cannot overlap'),
    )
