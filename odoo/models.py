    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def _sql_error_to_message_generic(self, exc: psycopg2.Error) -> str:
        ...
        if not columns:
            ...
            info['field_display'] = f"'{field.string}' ({field.name})"
        else:
...
class SQLObject:
    """ Declares a SQL object for the model. """
    key: str
    ...
    def _set_message(self, message: ConstraintMessageType):
        if isinstance(message, str):
            _logger.debug("Use a lazy translation or a function message for %r", self)
...
class Constraint(SQLObject):
    ...
    def __init__(
        ...
        is the SQL that will be used to add the constraint.
