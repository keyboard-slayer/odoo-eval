    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if (
                # use get to avoid KeyError when properties field is used
                isinstance(model_fields.get(field_path[0]), odoo.fields.Many2one) and
                # this only applies for toplevel m2o (?) fields
                field_path[0] in (self.env.context.get('name_create_enabled_fields') or [])
            ):
    ...
    def _load_records_write(self, values):
        self.ensure_one()
        to_write = {}  # Defered the write to avoid using the old definition if it changed
        for fname in list(values):
            ...
            set_val = values.pop(fname)
            to_write[fname] = dict(self[fname], **field_converter(set_val, self))
        ...
        if to_write:
            # Because we don't know which properties was linked to which definition, we can know
            # clean properties (note that it is not mandatory, we can wait that client change the record in a Form view)
