...
def create_index(
    ...
    *,
    ...
    :param nulls_distinct: Whether nulls should be distinct for unique indexes
    :param unique: Whether the index is unique or not (default: False)
    """
    assert nulls_distinct or unique, "Nulls can be distinct only for unique indexes"
    ...
    definition = SQL(
        ...
        SQL(" NULLS NOT DISTINCT") if not nulls_distinct else SQL(),
...
def create_unique_index(cr, indexname, tablename, expressions, *, nulls_distinct=True):
    """ Create the given index unless it exists. """
    warnings.warn("Since 18.0, use create_index(unique=True)", DeprecationWarning)
    return create_index(cr, indexname, tablename, expressions, unique=True, nulls_distinct=nulls_distinct)
