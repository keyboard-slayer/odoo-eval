...
def model(method: T) -> T:
    ...
    if method.__name__ == 'create':  # type: ignore
        return model_create_single(method)  # type: ignore
...
def model_create_multi(method: Callable[[Sequence[Mapping]], T]) -> Callable[[Sequence[Mapping] | Mapping], T]:
...
class Environment(Mapping[str, typing.Any]):
    ...
    @typing.overload
    def protecting(self, what: Collection[Field], records: BaseModel) -> Iterator[None]:
        ...

    @typing.overload
    def protecting(self, what: Collection[tuple[Field, BaseModel]]) -> Iterator[None]:
        ...

    @contextmanager
    def protecting(self, what, records: BaseModel | None = None):  # XXX typing context manager
...
class Transaction:
    ...
    __slots__ = ('cache', 'envs', 'protected', 'registry', 'tocompute')
...
NOTHING = object()  # XXX type: import sentinel from misc?
...
class Cache:
    ...
    def get(self, record: BaseModel, field: Field, default=NOTHING):
