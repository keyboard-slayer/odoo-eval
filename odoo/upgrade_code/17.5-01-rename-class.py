...
def get_class_name_models(file):
    ...
    while line is not None:
        class_name = line.startswith('class ') and ('Model)' in line or '.' in line) and line[6:].split(':')[0].split('(')[0]
        line = next(iter_lines, None)
...
def update_class_name(content, class_name_models):
    # update class name
    lines = content.split('\n')
    for old_class_name, (new_class_name, model_name) in class_name_models.items():
        if old_class_name == new_class_name:
            continue
        if old_class_name == new_class_name or class_name_to_model_name(old_class_name) == model_name:
