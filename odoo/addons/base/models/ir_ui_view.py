...
* if False, the view currently does not extend its parent but can be enabled
    ...
    _inheritance_mode = models.Constraint(
        ...
        lambda env: env._("Invalid inheritance mode: if the mode is 'extension', the view must extend an other view"),
    )
    _qweb_required_key = models.Constraint(
        "CHECK (type != 'qweb' OR key IS NOT NULL)",
        lambda env: env._('Invalid key: QWeb view should have a key'),
    )
