...
class IrEmbeddedActions(models.Model):
    ...
    _check_only_one_action_defined = models.Constraint(
        ...
        lambda env: env._('Constraint to ensure that either an XML action or a python_method is defined, but not both.'),
    )
    _check_python_method_requires_name = models.Constraint(
        'CHECK(\n                NOT (python_method IS NOT NULL AND name IS NULL)\n            )',
        lambda env: env._('Constraint to ensure that if a python_method is defined, then the name must also be defined.'),
    )
