...
class IrModel(models.Model):
    ...
    _obj_name_uniq = models.Constraint('UNIQUE (model)', _lt('Each model must have a unique name.'))
...
class IrModelFields(models.Model):
    ...
    _name_unique = models.Constraint('UNIQUE(model, name)', _lt("Field names must be unique per model."))
    _size_gt_zero = models.Constraint('CHECK (size>=0)', _lt('Size of the field cannot be negative.'))
    _name_manual_field = models.Constraint("CHECK (state != 'manual' OR name LIKE 'x\\_%')",
        _lt("Custom fields must have a name that starts with 'x_'!")
    )
...
class ModelInherit(models.Model):
    ...
    _uniq = models.Constraint("UNIQUE(model_id, parent_id)", _lt("Models inherits from another only once"))
...
class IrModelSelection(models.Model):
    ...
    _selection_field_uniq = models.Constraint(
        ...
        _lt('Selections values must be unique per field'),
    )
...
class IrModelConstraint(models.Model):
    ...
    _module_name_uniq = models.Constraint('UNIQUE (name, module)',
        _lt('Constraints with the same name are unique per module.'))
    ...
    def _reflect_model(self, model):
        ...
        for cons in model._sql_objects.values():
            ...
            message = cons.get_error_message(model.with_context(lang='en_US')) or None
...
class IrModelData(models.Model):
    ...
    _name_nospaces = models.Constraint("CHECK(name NOT LIKE '% %')", lambda env: env._("External IDs cannot contain spaces"))
