...
class IrFilters(models.Model):
    ...
    _name_model_uid_unique = models.Constraint(
        ...
        lambda env: env._('Filter names must be unique'),
    )
    _check_res_id_only_when_embedded_action = models.Constraint(
        ...
        lambda env: env._('Constraint to ensure that the embedded_parent_res_id is only defined when a top_action_id is defined.'),
    )
    _check_sort_json = models.Constraint(
        "CHECK(sort IS NULL OR jsonb_typeof(sort::jsonb) = 'array')",
        lambda env: env._('Invalid sort definition'),
    )
