...
class IrHttp(models.AbstractModel):
    ...
    def _auth_method_bearer(cls):
        ...
        def get_http_authorization_bearer_token():
            ...
            if authorization and authorization.type == 'bearer':
                return getattr(authorization, 'token', authorization.get('token'))
            # werkzeug<2.3 doesn't expose `authorization.token` (for bearer authentication), check header directly
        ...
        def check_sec_headers():
            """Protection against CSRF attacks in for requests.
            ...
            return (
                ...
                and headers.get("Sec-Fetch-Site") in ('none', 'same-site')
