...
class Currency(models.Model):
    ...
    _unique_name = models.Constraint(
        'unique (name)',
        lambda env: env._('The currency code must be unique!'),
    )
    _rounding_gt_zero = models.Constraint(
        'CHECK (rounding>0)',
        lambda env: env._('The rounding factor must be greater than 0!'),
    )
...
class CurrencyRate(models.Model):
    ...
    _unique_name_per_day = models.Constraint(
        ...
        lambda env: env._('Only one currency rate per day allowed!'),
    )
    _currency_rate_check = models.Constraint(
        'CHECK (rate>0)',
        lambda env: env._('The currency rate must be strictly positive.'),
    )
