...
from .tools import (
    float_repr, float_round, float_compare, float_is_zero, get_lang, human_size,
    OrderedSet, Query, sql, SQL, date_utils, unique,
...
class Field(MetaField('DummyField', (object,), {}), typing.Generic[T]):
    ...
    def field_to_sql(self, model: BaseModel, alias: str, flush: bool = True) -> SQL:
        ...
        return SQL.identifier(alias, self.name, to_flush=field_to_flush)
    ...
    def condition_to_sql(self, model: BaseModel, alias: str, field_expr: str, operator: str, value, query: Query, *, _value_to_column=None) -> SQL:
        ...
        if _value_to_column is None:
            _value_to_column = lambda v: self.convert_to_column(v, model, validate=False)  # noqa: E731
...
class Boolean(Field[bool]):
    ...
    def condition_to_sql(self, model: BaseModel, alias: str, field_expr: str, operator: str, value, query: Query) -> SQL:
        ...
        if len(value) != 1:
            return SQL("TRUE") if bool(value) == (operator == 'in') else SQL("FALSE")
...
class _String(Field[str | typing.Literal[False]]):
    ...
    def condition_to_sql(self, model: BaseModel, alias: str, field_expr: str, operator: str, value, query: Query, *, _value_to_column=None) -> SQL:
        # build the condition
        if _value_to_column is None and self.translate:
            _value_to_column = lambda v: self.convert_to_column(v, model, validate=False).adapted['en_US']  # noqa: E731
        base_condition = super().condition_to_sql(model, alias, field_expr, operator, value, query, _value_to_column=_value_to_column)
        ...
        if (
            ...
            sql_right = SQL("%s", _value_to_column(value))
